 ...
 * Lấy State và Function từ Redux (useDispatch) thay vì Context API.
 */
const ProductCard = ({ id, title, price, originalPrice, image, colorPlaceholder = 'bg-surface_dim', tag, description, category }) => {
  ...
  return (
    <Card className="group flex flex-col h-full relative">
      <Link to={`/shop/${id}`} className="block relative" aria-label={`View details for ${title}`}>
        <div
        ...
        >
          {displayImage ? (
            <img src={displayImage} alt="" aria-hidden="true" loading="lazy" className="w-full h-full object-cover" />
          ) : (
            <span className="text-surface_dim/20 font-black text-4xl opacity-10 select-none" aria-hidden="true">CANDY</span>
          )}

          {/* Tag overlay utilizing Pure Component */}
          {displayTag && (
            <PureBadge
              variant={tagVariant}
